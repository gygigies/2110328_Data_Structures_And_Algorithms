...
void add(int value,int pos,int level,int n,CP::map_bst<int,int> &bst){
  bst[value] = pos;
  if(level >= 0){
  add(value-pow(2,level),pos+pow(2,n-level),level-1,n,bst);
  add(value+pow(2,level),pos+pow(2,n-level)+1,level-1,n,bst);
  }
}
